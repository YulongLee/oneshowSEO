...
export async function notificationService(){await ensureExecutionSchema();repository??=new SqliteNotificationRepository(getDatabase());repository.ensureSchema();const secret=process.env.NOTIFICATION_RECOVERY_SIGNING_SECRET;return service??=new NotificationService(repository,secret?new RecoveryLinkSigner(secret):null);}
