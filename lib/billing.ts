...
import { publicPaymentState } from "./payment-providers";
import { publicProductPlans } from "./plan-pricing";
...
export const billingPlans = Object.fromEntries(
  (Object.keys(planCatalog) as PlanKey[]).map((key) => [key, publicPlan(key)]),
) as Record<PlanKey, BillingPlan>;
export async function configuredBillingPlans() {
  return publicProductPlans();
}
...
export async function ensureBillingSchema(): Promise<void> {
  ...
  for (const table of [
    ...
    database.exec(
      `CREATE INDEX IF NOT EXISTS ${table}_organization_idx ON ${table}(organization_id,created_at)`,
    );
  }
  database.exec(`
  ...
  for (const plan of Object.values(planCatalog)) {
    ...
    const existing = database
      .prepare(
        `SELECT price_version AS priceVersion,monthly_price_cents AS monthlyPriceCents,entitlements_json AS entitlementsJson FROM commerce_plan_versions WHERE plan_key=? AND catalog_version=? AND currency=?`,
      )
      .bind(plan.key, plan.catalogVersion, plan.currency)
      .first<{
        priceVersion: string;
        monthlyPriceCents: number;
    ...
    if (
      existing &&
      (existing.priceVersion !== plan.priceVersion ||
        existing.monthlyPriceCents !== plan.monthlyPriceCents ||
        existing.entitlementsJson !== entitlements)
    )
      throw new Error(
        `PLAN_CATALOG_VERSION_CONFLICT:${plan.key}:${plan.catalogVersion}`,
      );
    database
      .prepare(
        `UPDATE commerce_plan_versions SET active=0 WHERE plan_key=? AND currency=? AND catalog_version<>?`,
      )
      .bind(plan.key, plan.currency, plan.catalogVersion)
      .run();
    database
      .prepare(
        `INSERT OR IGNORE INTO commerce_plan_versions(plan_key,catalog_version,price_version,currency,monthly_price_cents,entitlements_json,active,created_at) VALUES (?,?,?,?,?,?,1,?)`,
      )
      .bind(
        plan.key,
        plan.catalogVersion,
    ...
    database
      .prepare(
        `UPDATE commerce_plan_versions SET active=1 WHERE plan_key=? AND catalog_version=? AND currency=?`,
      )
      .bind(plan.key, plan.catalogVersion, plan.currency)
      .run();
  }
...
export function commerceRepository() {
  return (repository ??= new SqliteCommerceRepository(getDatabase()));
}
export function commerceService() {
  return (service ??= new CommercialEntitlementService(commerceRepository()));
}
export function paymentRepository() {
  return (paymentRepositoryInstance ??= new SqlitePaymentRepository(
    getDatabase(),
  ));
}
export function sandboxPaymentProvider() {
  return (sandboxProviderInstance ??= new SandboxPaymentProvider());
}
export function sandboxPaymentService() {
  const secret = process.env.BILLING_SANDBOX_WEBHOOK_SECRET;
  if (process.env.BILLING_SANDBOX_ENABLED !== "true" || !secret) return null;
...
export function commercialSubject(user: AppUser): CommercialSubject {
  return {
    accountId: user.id,
...
export async function billingPaymentState() {
  return publicPaymentState();
}
