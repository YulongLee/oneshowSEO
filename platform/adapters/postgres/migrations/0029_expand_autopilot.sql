...
CREATE TABLE agents.autopilot_configs (project_id text PRIMARY KEY REFERENCES project_governance.projects(id) ON DELETE CASCADE,organization_id text NOT NULL REFERENCES identity.organizations(id) ON DELETE CASCADE,enabled boolean NOT NULL DEFAULT false,cron text NOT NULL DEFAULT '0 3 * * *',timezone text NOT NULL DEFAULT 'Asia/Shanghai',daily_credit_limit integer NOT NULL DEFAULT 43 CHECK (daily_credit_limit BETWEEN 23 AND 500),content_enabled boolean NOT NULL DEFAULT true,paused_at timestamptz,next_run_at timestamptz,revision bigint NOT NULL DEFAULT 1,updated_by_account_id text REFERENCES identity.accounts(id) ON DELETE SET NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX autopilot_configs_due_idx ON agents.autopilot_configs(enabled,paused_at,next_run_at);
CREATE TABLE agents.autopilot_runs (id text PRIMARY KEY,organization_id text NOT NULL REFERENCES identity.organizations(id) ON DELETE CASCADE,project_id text NOT NULL REFERENCES project_governance.projects(id) ON DELETE CASCADE,scheduled_for timestamptz NOT NULL,status text NOT NULL CHECK(status IN('running','completed','partial','failed','paused')),credit_limit integer NOT NULL,credits_planned integer NOT NULL DEFAULT 0,credits_used integer NOT NULL DEFAULT 0,strategy_summary jsonb,started_at timestamptz NOT NULL DEFAULT now(),completed_at timestamptz,error_code text,UNIQUE(project_id,scheduled_for));
