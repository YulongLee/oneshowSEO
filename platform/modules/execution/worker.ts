...
export type WorkerTerminalState="completed"|"failed"|"cancelled"|"quarantined";
export type WorkerHandler={
  authorize:(context:WorkerAuthorizationContext)=>Promise<void>|void;
  execute:(input:Record<string,unknown>,context:WorkerHandlerContext)=>Promise<unknown>;
  settle?:(input:{state:WorkerTerminalState;result:unknown;error:unknown|null},context:WorkerAuthorizationContext)=>Promise<void>|void;
};
...
export class ExecutionWorkerSupervisor{
  ...
  private async execute(claim:WorkerClaim,tokenHash:string,controller:AbortController){const handler=this.handlers[claim.job.jobType];if(!handler)throw new Error("WORKER_HANDLER_NOT_FOUND");const heartbeatTimer=setInterval(()=>this.heartbeat(claim,tokenHash,controller),this.options.heartbeatIntervalMs),timeoutTimer=setTimeout(()=>controller.abort(new WorkerJobError("JOB_TIMEOUT","Job execution timed out",true)),claim.job.timeoutSeconds*1000),clear=()=>{clearInterval(heartbeatTimer);clearTimeout(timeoutTimer);};controller.signal.addEventListener("abort",clear,{once:true});
    try{try{await handler.authorize({task:claim.task,job:claim.job,attemptId:claim.attempt.id});}catch(error){throw new WorkerJobError("EXECUTION_NOT_AUTHORIZED",safeError(error),false);}const result=await handler.execute(claim.task.input,{task:claim.task,job:claim.job,attemptId:claim.attempt.id,signal:controller.signal,heartbeat:()=>this.heartbeat(claim,tokenHash,controller)});if(controller.signal.aborted)throw controller.signal.reason;if(!this.heartbeat(claim,tokenHash,controller))throw controller.signal.reason;if(!this.repository.succeedAttempt({claim,workerId:this.options.workerId,tokenHash,now:this.now()}))throw new WorkerLeaseLostError();await handler.settle?.({state:"completed",result,error:null},{task:claim.task,job:claim.job,attemptId:claim.attempt.id});}
    catch(error){const reason=controller.signal.aborted?controller.signal.reason:error;if(reason instanceof WorkerShutdownError||reason instanceof WorkerLeaseLostError)return;if(reason instanceof WorkerCancellationError){const changed=this.repository.cancelAttempt({claim,workerId:this.options.workerId,tokenHash,now:this.now(),reason:safeError(reason)});if(!changed)throw reason;await handler.settle?.({state:"cancelled",result:null,error:reason},{task:claim.task,job:claim.job,attemptId:claim.attempt.id});return;}const normalized=reason instanceof WorkerJobError?reason:new WorkerJobError("WORKER_UNEXPECTED_ERROR",safeError(reason),true),canRetry=normalized.retryable&&claim.attempt.attemptNumber<claim.job.maxAttempts,state:WorkerFailureState=canRetry?"retrying":normalized.retryable?"quarantined":"failed",retryAt=canRetry?this.now()+this.backoff(claim.attempt.attemptNumber):null,changed=this.repository.failAttempt({claim,workerId:this.options.workerId,tokenHash,now:this.now(),state,retryAt,errorCode:normalized.code.slice(0,128),errorMessage:safeError(normalized)});if(!changed)throw normalized;if(state!=="retrying")await handler.settle?.({state,result:null,error:normalized},{task:claim.task,job:claim.job,attemptId:claim.attempt.id});}
