...
export function CommercialUsers({notify}:{notify:(message:string)=>void}){
  ...
  const load=async()=>{setLoading(true);try{const response=await fetch("/api/admin/users");const data=await response.json() as {users?:ManagedUser[];error?:string};if(!response.ok)throw new Error(data.error);setUsers(data.users||[])}catch(error){notify(error instanceof Error?error.message:"用户数据加载失败")}finally{setLoading(false)}};
  useEffect(()=>{load()},[]);
  const shown=useMemo(()=>users.filter(user=>`${user.name} ${user.email}`.toLowerCase().includes(query.toLowerCase())),[users,query]);
  async function update(user:ManagedUser,field:"role"|"status"|"plan",value:string){const response=await fetch("/api/admin/users",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({id:user.id,[field]:value})});const result=await response.json() as {error?:string};if(!response.ok){notify(result.error||"更新失败");return}setUsers(current=>current.map(item=>item.id===user.id?{...item,[field]:value}:item) as ManagedUser[]);notify(`${user.name} 的账号设置已更新`)}
