...
export async function PATCH(request:Request){
 ...
 if(!result.meta.changes){const next=body.status==="approved"?"queued":"cancelled";const changed=db.transaction(()=>{const update=db.prepare(`UPDATE execution_tasks SET state=?,cancellable=?,updated_at=?,completed_at=?,version=version+1 WHERE id=? AND organization_id=? AND state='waiting_approval' AND project_id IN (SELECT id FROM projects WHERE organization_id=? AND status='active')`).bind(next,next==="queued"?1:0,now,next==="cancelled"?now:null,body.id,user.organization.organizationId,user.organization.organizationId).run();if(update.meta.changes&&next==="cancelled")db.prepare("UPDATE execution_jobs SET state='cancelled',updated_at=?,completed_at=? WHERE organization_id=? AND task_id=? AND state IN ('queued','retrying')").bind(now,now,user.organization.organizationId,body.id).run();return update.meta.changes;});if(!changed)return NextResponse.json({error:"任务不存在或状态已变化"},{status:409});}
