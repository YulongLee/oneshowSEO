...
export async function GET(request:Request,context:{params:Promise<{id:string}>}){
 ...
 const run=db.prepare(`SELECT id,score,pages_scanned AS pagesScanned,urls_discovered AS urlsDiscovered,checks_total AS checksTotal,checks_passed AS checksPassed,checks_warning AS checksWarning,checks_failed AS checksFailed,checks_unknown AS checksUnknown,checks_skipped AS checksSkipped,evidence_count AS evidenceCount,coverage_status AS coverageStatus,partial_reasons AS partialReasonsJson,degraded_sources AS degradedSourcesJson,agent_version AS agentVersion,completed_at AS completedAt FROM audit_runs WHERE project_id=? AND status='completed' ORDER BY started_at DESC LIMIT 1`).bind(id).first<{id:string;partialReasonsJson:string;degradedSourcesJson:string;partialReasons?:string[];degradedSources?:string[]}>();
 ...
 try{run.partialReasons=JSON.parse(run.partialReasonsJson||"[]")}catch{run.partialReasons=[]}try{run.degradedSources=JSON.parse(run.degradedSourcesJson||"[]")}catch{run.degradedSources=[]}const data={project,run,checks,scores,pages} as unknown as AuditReportData;const format=new URL(request.url).searchParams.get("format")==="markdown"?"markdown":"html";
