...
const failure=(error:unknown)=>{if(error instanceof InvitationError||error instanceof AuthorizationError||error instanceof CommerceError)return NextResponse.json({error:error.message,code:error.code},{status:error.status});throw error;};
...
export async function POST(request:Request){const user=await getCurrentUser();if(!user)return NextResponse.json({error:"请先登录"},{status:401});try{
 ...
 await ensureBillingSchema();const db=getDatabase();const active=db.prepare("SELECT COUNT(*) AS total FROM identity_memberships WHERE organization_id=? AND status='active'").bind(user.organization.organizationId).first<{total:number}>()?.total??0;const pending=db.prepare("SELECT COUNT(*) AS total FROM identity_invitations WHERE organization_id=? AND status='pending' AND expires_at>?").bind(user.organization.organizationId,Math.floor(Date.now()/1000)).first<{total:number}>()?.total??0;const effective=commerceService().authorize(commercialSubject(user),"seats",1,active+pending);
 const created=await services().invitations.invite({organizationId:user.organization.organizationId,email:body?.email,role:body?.role,projectScope:body?.projectScope,invitedByUserId:user.id,seatLimit:effective.limits.seats});
