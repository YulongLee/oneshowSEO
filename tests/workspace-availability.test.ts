...
test("unconnected prototype modules are explicitly labelled demo or unavailable", () => {
  ...
  for(const key of ["关键词研究","数据分析","内容库","知识库","排名监控","AI 可见性"]) assert.equal(states[key]?.state,"demo",key);
...
test("connected sources promote only their owned modules to real data", () => {
  const states=workspaceAvailability({capturedAt:1,hasAudit:true,hasResearch:true,hasContentWorkflow:true,hasGeo:true,hasKeywordMetrics:true,hasSearchPerformance:true,hasAnalytics:true,hasRankProvider:true,hasCustomerIntegrations:true,billingLive:true,apiEnabled:true});
