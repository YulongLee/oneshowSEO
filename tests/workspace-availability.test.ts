...
test("unconnected modules use honest empty or unavailable states", () => {
  ...
  for(const key of ["关键词研究","内容库","知识库","排名监控","AI 可见性"]) assert.equal(states[key]?.state,"no_data",key);
  for(const key of ["Billing","套餐升级"]) assert.equal(states[key]?.state,"unavailable",key);
  assert.equal(states["Agent Center"]?.state,"fresh");
...
test("connected sources promote only their owned modules to real data", () => {
  ...
  assert.equal(states["AI 可见性"]?.state,"fresh");
});
